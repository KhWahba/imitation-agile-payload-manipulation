...
the new Planner object bindings (`pcdbcbs_planner` / `pcdbcbs_planner_dbg`).

Additional logging vs the original memprobe wrapper:
  - planner wrapper stats snapshot after each expert call (if available)
"""
...
def main() -> None:
    ...
    if MEMPROBE_USE_DBG_EXPERT:
        import expert_pcdbcbs_planner_dbg as expert_mod
        expert_base_cls = expert_mod.PcDbCBSExpert
    else:
        import expert_pcdbcbs_planner as expert_mod
        expert_base_cls = expert_mod.PcDbCBSExpert
    ...
    class InstrumentedPcDbCBSExpert(expert_base_cls):  # type: ignore[misc]
        ...
        def act(self, obs):
            ...
            try:
                ...
                planner_stats_fn = getattr(self, "get_planner_stats_dict", None)
                if callable(planner_stats_fn):
                    try:
                        ps = planner_stats_fn() or {}
                        for k, v in ps.items():
                            record[f"planner_{k}"] = v
                    except Exception as e:
                        record["planner_stats_error"] = repr(e)

                logger.log(record)

                delta_mb = (record["rss_delta_kb"] or 0) / 1024.0
                planner_delta = record.get("planner_last_rss_delta_kb")
                print(
                    "[MEMPROBE-PLANNER] "
                    ...
                    f"planner_delta_kb={planner_delta} "
                    f"hwm={hwm_after}",
                    flush=True,
                )
    ...
    if MEMPROBE_USE_DBG_EXPERT:
        print("[MEMPROBE-PLANNER] using debug planner binding module", flush=True)
    else:
        print("[MEMPROBE-PLANNER] using planner binding module", flush=True)
    if MEMPROBE_REPLAN_K is not None:
        print(f"[MEMPROBE-PLANNER] overriding dagger replan_every_k -> {int(MEMPROBE_REPLAN_K)}", flush=True)
    print(f"[MEMPROBE-PLANNER] logging expert-call memory to {LOG_PATH}", flush=True)
    base.main()
