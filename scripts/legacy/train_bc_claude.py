...
def collect_expert_trajs_and_logs(
    ...
    *,
    ...
    for ep in range(n_episodes):
        ...
        # reset expertâ€™s internal plan cache
...
def main():
    ...
    venv = DummyVecEnv([lambda: make_env(max_steps=500)])
    # --- expert (policy-space actions in [-1,1]) ---
    tmp_env = PayloadGymEnv(xml_path=xml_path, template_yaml_path=template_yaml, max_steps=500)
    ...
    # --- cache expert trajs ---
    out_root = Path("runs/bc_payload_v0")
    ...
    # IMPORTANT: if obs dimension changed (e.g. added prev_action), you MUST
    # delete the cached .pkl so trajectories are re-collected with the new obs layout.
    if trajs_pkl.exists():
        with open(trajs_pkl, "rb") as f:
            expert_trajs = pickle.load(f)
        # Validate obs dimension matches current env
        expected_obs_dim = venv.observation_space.shape[0]
        cached_obs_dim = expert_trajs[0].obs.shape[1]
        if cached_obs_dim != expected_obs_dim:
            print(f"WARNING: cached obs dim {cached_obs_dim} != expected {expected_obs_dim}. "
                  f"Deleting stale cache and re-collecting.")
            trajs_pkl.unlink()
            expert_trajs = None
        else:
            print(f"Loaded cached expert_trajs: {trajs_pkl} (n={len(expert_trajs)}, obs_dim={cached_obs_dim})")

    if not trajs_pkl.exists():
        expert_trajs, _ = collect_expert_trajs_and_logs(
            env_fn=lambda: make_env(max_steps=500),
            expert=expert,
            n_episodes=10,  # <-- increase this first; 10 can be too low
            xml_path=xml_path,
    ...
    # --- BC trainer (make policy explicit so itâ€™s stable across versions) ---
    bc_trainer = bc.BC(
    ...
    bc_ret, _ = evaluate_policy(bc_trainer.policy, venv, n_eval_episodes=1, deterministic=True)
    ex_ret, _ = evaluate_policy(expert_policy, venv, n_eval_episodes=1, deterministic=True)
