#!/usr/bin/env python3
"""
Run train_dagger_payload.py with expert-call memory instrumentation.

This keeps the main training script unchanged and only wraps PcDbCBSExpert.act()
to log:
  - RSS / HWM before and after each expert call
  - call duration
  - whether the call triggered a replan
  - expert mode (seed vs dagger, inferred from replan_every_k)
...
def main() -> None:
    ...
    expert_base_cls = base.PcDbCBSExpert
    if MEMPROBE_USE_DBG_EXPERT:
        import expert_pcdbcbs_dbg as expert_dbg
        expert_base_cls = expert_dbg.PcDbCBSExpert
    ...
    # Patch the class symbol used inside train_dagger_payload.main()
    base.PcDbCBSExpert = InstrumentedPcDbCBSExpert

    if MEMPROBE_USE_DBG_EXPERT:
        print("[MEMPROBE] using debug expert binding module (pcdbcbs_dbg)", flush=True)
    if MEMPROBE_REPLAN_K is not None:
        print(f"[MEMPROBE] overriding dagger replan_every_k -> {int(MEMPROBE_REPLAN_K)}", flush=True)
    print(f"[MEMPROBE] logging expert-call memory to {LOG_PATH}", flush=True)
    base.main()
